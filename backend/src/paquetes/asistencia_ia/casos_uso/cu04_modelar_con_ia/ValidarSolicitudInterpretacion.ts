...
function esModelo(valor: unknown): valor is ModeloUMLCanonicoIA {
  ...
  const clasesValidas = valor.clases.every((candidata) => {
    ...
    const atributosValidos = candidata.atributos.every((atributo) => objeto(atributo) && typeof atributo.id === "string" && typeof atributo.nombre === "string" && (typeof atributo.tipo === "string" || atributo.tipo === null) && (atributo.visibilidad === undefined || atributo.visibilidad === "publica" || atributo.visibilidad === "privada" || atributo.visibilidad === "protegida" || atributo.visibilidad === "paquete"))
    const metodosValidos = candidata.metodos === undefined || (Array.isArray(candidata.metodos) && candidata.metodos.every((metodo) => objeto(metodo) && typeof metodo.id === "string" && typeof metodo.nombre === "string" && (metodo.visibilidad === "publica" || metodo.visibilidad === "privada") && typeof metodo.tipoRetorno === "string" && Array.isArray(metodo.parametros) && metodo.parametros.every((parametro) => objeto(parametro) && typeof parametro.id === "string" && typeof parametro.nombre === "string" && typeof parametro.tipo === "string")))
    return atributosValidos && metodosValidos
  })
