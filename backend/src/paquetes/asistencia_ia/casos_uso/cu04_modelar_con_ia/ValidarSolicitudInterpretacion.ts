...
function esModelo(valor: unknown): valor is ModeloUMLCanonicoIA {
  ...
  const clasesValidas = valor.clases.every((candidata) => {
    ...
    const atributosValidos = candidata.atributos.every((atributo) => objeto(atributo) && typeof atributo.id === "string" && typeof atributo.nombre === "string" && (typeof atributo.tipo === "string" || atributo.tipo === null) && (atributo.visibilidad === undefined || atributo.visibilidad === "publica" || atributo.visibilidad === "privada"))
