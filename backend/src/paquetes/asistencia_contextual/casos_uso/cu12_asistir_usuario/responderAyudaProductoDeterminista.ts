...
export function responderAyudaProductoDeterminista(solicitud: SolicitudAsistenteContextual): RespuestaAsistenteContextual | null {
  ...
  if (/(completo|completitud|que podria faltar|que falta|que mas.*(agregar|anadir)|otras entidades|que atributos (deberia|tendria|le faltan)|complete|completeness|what is missing|what else.*add|other entities|what attributes.*(should|would))/.test(pregunta)) {
    return crearRespuesta(ingles
      ? `Result:\n${resumenEstado(solicitud)}\n\nBusiness completeness cannot be proven from the UML diagram alone. SW1 can determine structural validity and Spring readiness. To evaluate functional completeness, provide the business requirements or use cases.`
      : `Resultado:\n${resumenEstado(solicitud)}\n\nLa completitud del negocio no puede demostrarse solo con el diagrama UML. SW1 puede determinar validez estructural y aptitud para Spring. Para evaluar completitud funcional, proporciona los requisitos del negocio o casos de uso.`)
  }

  if (/(asistente contextual|contextual assistant|que puedes hacer|what can you do)/.test(pregunta)) {
    return crearRespuesta(ingles
      ? "The Contextual assistant teaches how to use SW1 and explains objective facts from the current UML, selection, validation and generation status. It is advisory: it never changes or saves the model, imports or exports XMI, or generates the backend by itself."
  ...
  if (/(asistente ia|ia de modelado|crear clase con ia|modificar.* ia|modeling ai|create.*with ai|use ai)/.test(pregunta)) {
    return crearRespuesta(ingles
      ? "How to use it:\n1. Open Modeling AI from the left rail.\n2. Write a UML instruction, for example: Create a Cliente class with id Long and correo String.\n3. Press Send.\n4. SW1 interprets it against the current canonical UML and automatically applies a clear, valid incremental change."
      : `Cómo hacerlo:\n1. Abre ${CONTEXTO_PRODUCTO_SW1.capacidades.iaModelado.acceso}.\n2. Escribe una instrucción UML, por ejemplo: Crea una clase Cliente con id Long y correo String.\n3. Pulsa Enviar.\n4. SW1 la interpreta sobre el UML canónico actual y aplica automáticamente un cambio incremental claro y válido.`)
  }
