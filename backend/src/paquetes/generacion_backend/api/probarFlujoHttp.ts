...
const salida = resolve("generated-test-output", "api-proof")
...
try {
  ...
  const respuesta = await fetch(`http://127.0.0.1:${direccion.port}/api/generacion/spring`, {
    ...
    body: JSON.stringify(fixtureCliente),
  })
  ...
  console.log(`ZIP HTTP guardado en ${rutaZip}`)
  console.log(`Proyecto extraído en ${join(rutaExtraccion, "backend-generado")}`)
} finally {
