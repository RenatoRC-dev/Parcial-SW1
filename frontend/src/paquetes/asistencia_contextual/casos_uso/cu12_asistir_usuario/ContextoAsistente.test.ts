...
import { construirContextoAsistente, obtenerOrientacionDeterminista } from "./ContextoAsistente"
...
function contexto(modelo: ModeloUMLCanonico, candidatoImagenPendiente = false) {
  const validacion = validarModelo(modelo)
  return construirContextoAsistente({ proyectoId: "p", modelo, validacion, aptitud: evaluarAptitudGeneracionSpring(modelo, validacion), cambiosSinGuardar: true, candidatoImagenPendiente })
}
...
describe("contexto determinista de CU12", () => {
  it("distingue UML válido de un N:M que bloquea generación", () => {
    ...
    expect(JSON.stringify(actual)).not.toContain("atributos")
  })
